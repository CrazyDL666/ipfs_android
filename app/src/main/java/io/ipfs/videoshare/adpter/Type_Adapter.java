...
public class Type_Adapter extends RecyclerView.Adapter<Type_Adapter.MyViewHolder>{
    ...
    public interface OnRecyclerviewItemClickListener {
        void onItemClickListener(View v, String hash,String title);
    }

    public Type_Adapter(Context context, UserBean userBean, OnRecyclerviewItemClickListener mOnRecyclerviewItemClickListener){
...
//最后一个,应该加载Footer
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup viewGroup, final int viewType ) {
        ...
        View view = inflater.inflate(R.layout.list_item2, viewGroup, false);
            ...
            if(holder instanceof MyViewHolder) {
                ...
                ((MyViewHolder) holder).kuai.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        mOnRecyclerviewItemClickListener.onItemClickListener(v,userBean.getType().get(position).getName(),userBean.getType().get(position).getTitle());
                    }
                });
                holder.title.setText(userBean.getType().get(position).getTitle());
                return;
            }
            return;
    ...
    public int getItemCount() {
        if(mHeaderView == null && mFooterView == null){
            return userBean.getType().size();
        }else if(mHeaderView == null && mFooterView != null){
            return userBean.getType().size() + 1;
        }else if (mHeaderView != null && mFooterView == null){
            return userBean.getType().size() + 1;
        }else {
            return userBean.getType().size() + 2;
        }
    }
